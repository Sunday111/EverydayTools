...
template <
    typename TypesTuple,
    typename RowsValues,
    typename ColumnValues,
    typename StartRowValues = RowsValues,
    typename StartColumnValues = ColumnValues>
struct TemplateIterator
{
    static constexpr bool HasNext = HasNext_v<TypesTuple> || HasNext_v<RowsValues> || HasNext_v<ColumnValues>;
    ...
    using IncrementType =
        std::conditional_t<HasNext_v<TypesTuple>, TemplateIterator<TypesTail, StartRowValues, StartColumnValues>, void>;
    ...
    using IncrementColumn = std::conditional_t<
        HasNext_v<ColumnValues>,
        TemplateIterator<TypesTuple, RowsValues, ColumnsTail, StartRowValues, StartColumnValues>,
        IncrementRow>;

    using Next = IncrementColumn;
};

template <template <typename, size_t, size_t> typename Target, typename Iterator>
...
template <template <typename, size_t, size_t> typename Target, size_t Rows, size_t Columns, typename... Types>
struct Combinator
{
    static void Execute()
    {
        ...
        using BaseColumnsSequence = decltype(std::make_index_sequence<Columns + 1>());
        using ColumnsSequence = typename IndexSequenceTraits<BaseColumnsSequence>::Tail;
        using Iterator = TemplateIterator<TypesTuple, RowsSequence, ColumnsSequence>;
...
struct Randomizer
{
    static void Randomize(T& value)
    {
        value = static_cast<T>(std::rand() % 256);
    }
};
...
struct InputGenerator<random, std::tuple<Types...>>
{
    ...
    static void Randomize(std::index_sequence<Indices...>, [[maybe_unused]] Result& result)
    {
...
template <typename T, size_t R, size_t C, template <typename, size_t, size_t> typename Target>
...
private:
    template <size_t... Indices>
    static void FoldExecuteArgs([[maybe_unused]] Input input, std::index_sequence<Indices...>)
    {
...
public:
    static void Execute()
    {
        if constexpr (FT::Enabled)
        {
            ...
            FoldExecuteArgs(Generator::Generate(), std::make_index_sequence<std::tuple_size_v<Input>>());
        }
    }
};
...
struct TestCaller
{
    ...
    static void Execute()
    {
        using FinalCombinator = Combinator<Adjusted, 6, 6, unsigned int, int, float, double>;
