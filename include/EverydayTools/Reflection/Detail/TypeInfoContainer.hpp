...
#include "TypeTraits/ClassTypeTraits.hpp"
#include "TypeTraits/CommonTypeTraits.hpp"
#include "TypeTraits/EnumerationTypeTraits.hpp"
#include "TypeTraits/NumberTypeTraits.hpp"


namespace edt::reflection::detail {
template <typename T>
using BitsetFlagsList = ValuesList_FilterT<
    TypeFlagHasTrait,
...
template <typename T>
using AllTypeFlagTraitsList =
    ValuesList_MapToTypeT<TypeFlagTraits, BitsetFlagsList<T>>;

template <auto bitset>
using BitsetValuesList =
    ValuesList_FilterT<FlagIsSetFilter<bitset>::template Functor,
                       BitsetFlagsList<decltype(bitset)>>;

template <auto bitset>
using BitsetTraitsList =
    ValuesList_MapToTypeT<TypeFlagTraits, BitsetValuesList<bitset>>;
...
class TypeInfoContainer : public BaseType<typeFlags>, public ITypeInfo {
 public:
  ...
  template <TypeFlags flag>
  using FlagContainerConstPtr =
      std::add_pointer_t<std::add_const_t<FlagContainer<flag>>>;
...
template <TypeFlags typeFlags>
template <TypeFlags flag>
typename TypeInfoContainer<typeFlags>::FlagContainerPtr<flag>
TypeInfoContainer<typeFlags>::GetFlagAssociatedContainer() {
  using ConstThisPtr =
      std::add_pointer_t<std::add_const_t<TypeInfoContainer<typeFlags>>>;
  return const_cast<FlagContainerPtr<flag>>(
      const_cast<ConstThisPtr>(this)->GetFlagAssociatedContainer<flag>());
}

template <TypeFlags typeFlags>
template <TypeFlags flag>
typename TypeInfoContainer<typeFlags>::FlagContainerConstPtr<flag>
TypeInfoContainer<typeFlags>::GetFlagAssociatedContainer() const {
  static_assert(edt::GetSetFlagsCount(flag) == 1,
                "You can get container associated with one flag only");
...
constexpr TypeFlags GetTypeFlags() {
  using ApplicableTraits = edt::ValuesList_FilterT<
      FlagIsDefaultApplicableFilter<T>::template Predicate,
      BitsetFlagsList<TypeFlags>>;
