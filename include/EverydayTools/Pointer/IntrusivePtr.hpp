...
edt::IntrusivePtr<U, typename Traits> static_pointer_cast(
    const edt::IntrusivePtr<T, typename Traits>& ip) {
  return ip.template StaticCast<U>();
}

template <typename U, typename T, typename Traits>
edt::IntrusivePtr<U, Traits> dynamic_pointer_cast(
    const edt::IntrusivePtr<T, Traits>& ip) {
