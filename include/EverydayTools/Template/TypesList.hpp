...
template <typename A, typename B>
using TypesList_ConcatT =
    decltype(TypesList_Concat(std::declval<A>(), std::declval<B>()));
...
using TypesList_MapToTypeT =
    decltype(TypesList_MapToType<F>(std::declval<List>()));
...
template <template <typename> typename F, typename List>
using TypesList_MapToValueT =
    decltype(TypesList_MapToValue<F>(std::declval<List>()));
...
struct TypeList_FilterImpl<Predicate, Head, Tail...> {
  using FilteredHead =
      std::conditional_t<Predicate<Head>::Value, TypesList<Head>, TypesList<>>;
...
using TypesList_FilterT =
    decltype(TypesList_Filter<Predicate>(std::declval<List>()));
}  // namespace edt