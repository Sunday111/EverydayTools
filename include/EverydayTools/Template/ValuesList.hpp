...
using ValuesList_ConcatT =
    decltype(ValuesList_Concat(std::declval<A>(), std::declval<B>()));
...
using ValuesList_MapToTypeT =
    decltype(ValuesList_MapToType<F>(std::declval<List>()));
...
template <template <auto> typename F, typename List>
using ValuesList_MapToValueT =
    decltype(ValuesList_MapToValue<F>(std::declval<List>()));
...
struct Impl<Predicate, Head, Tail...> {
  using FilteredHead = std::conditional_t<Predicate<Head>::Value,
                                          ValuesList<Head>, ValuesList<>>;
...
using ValuesList_FilterT =
    decltype(ValuesList_Filter<Predicate>(std::declval<List>()));
...
using ValuesList_ToIntegerSequenceT =
    decltype(ValuesList_ToIntegerSequence<T>(std::declval<List>()));

template <typename T, T... values>
constexpr auto ValuesList_FromIntegerSequence(
    std::integer_sequence<T, values...>) {
  return ValuesList<values...>{};
}

template <typename List>
using ValuesList_FromIntegerSequenceT =
    decltype(ValuesList_FromIntegerSequence(std::declval<List>()));
}  // namespace edt
