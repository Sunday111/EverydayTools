...
namespace edt::array_view_details
{
    ...
    [[nodiscard]]
    constexpr inline decltype(auto) GetMemberValue(T& t, TMember member) noexcept
    {
        return (t.*member);
    }
}
...
namespace edt
{
    ...
    {
    public:
        constexpr DenseRandomAccessIterator(T* ptr) noexcept :
            m_p(ptr)
        {}
        ...
        [[nodiscard]]
        constexpr bool TheSame(const DenseRandomAccessIterator& another) const noexcept
        {
    ...
    public:
        ...
        [[nodiscard]]
        constexpr decltype(auto) MakeMemberView(MemberPtr member) const noexcept
        {
         ...
         */
        [[nodiscard]]
        constexpr SparseArrayView<T> Split(size_t startIndex, size_t count) const
        {
    ...
    public:
         ...
         */
        [[nodiscard]]
        SparseArrayView<T> Split(size_t startIndex, size_t count) const noexcept
        {
            assert(startIndex + count <= m_size);
            return SparseArrayView<T>(
