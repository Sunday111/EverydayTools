...
namespace edt
{
    ...
    public:
        ...
        constexpr StringViewT(const T* p, size_t size)
            : m_p(p)
            , m_size(size)
        {}
        ...
        friend bool operator==(const T* a, const StringViewT& b) {
            return b == a;
        }

        friend std::ostream& operator<<(std::ostream& output, const StringViewT& view) {
