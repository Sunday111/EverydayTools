...
[[nodiscard]] constexpr inline decltype(auto) GetMemberValue(
    T& t, TMember member) noexcept {
  return (t.*member);
}
}  // namespace edt::array_view_details
...
class RandomAccessIterator {
 public:
  ...
  [[nodiscard]] friend bool operator<(const TFinal& a,
                                      const TFinal b) noexcept {
...
class SparseRandomAccessIterator
 ...
 public:
  constexpr SparseRandomAccessIterator(T* ptr, size_t stride) noexcept
      : m_p(ptr), m_stride(stride) {}
  ...
  [[nodiscard]] bool TheSame(
      const SparseRandomAccessIterator& another) const noexcept {
...
class DenseRandomAccessIterator
 ...
 public:
  constexpr DenseRandomAccessIterator(T* ptr) noexcept : m_p(ptr) {}
  ...
  [[nodiscard]] constexpr bool TheSame(
      const DenseRandomAccessIterator& another) const noexcept {
...
class SparseArrayView : public ArrayView<T, edt::SparseArrayView> {
 ...
 public:
  ...
  constexpr SparseArrayView(const SparseArrayView<U>& another) noexcept
      : SparseArrayView(another.GetData(), another.GetSize(),
                        another.GetStride()) {}
   ...
   */
  constexpr SparseArrayView(T* ptr = nullptr, size_t size = 0,
                            size_t stride = sizeof(T)) noexcept
      : m_p(ptr), m_size(size), m_stride(stride) {
    assert(stride >= sizeof(T));
   ...
   */
  template <typename MemberPtr, class = std::enable_if_t<
                                    std::is_member_object_pointer_v<MemberPtr>>>
  [[nodiscard]] constexpr decltype(auto) MakeMemberView(
      MemberPtr member) const noexcept {
    ...
    return SparseArrayView<MemberType>(&GetMemberValue(*GetData(), member),
                                       GetSize(), GetStride());
  }
   ...
   */
  [[nodiscard]] constexpr SparseArrayView<T> Split(size_t startIndex,
                                                   size_t count) const {
...
class DenseArrayView : public ArrayView<T, edt::DenseArrayView> {
 ...
 public:
   ...
   */
  constexpr DenseArrayView(T* ptr = nullptr, size_t size = 0) noexcept
      : m_p(ptr), m_size(size) {
    assert(size == 0 || ptr != nullptr);
  }
  ...
  /*
      ...
      @param count - the number of elements in result
   */
  [[nodiscard]] SparseArrayView<T> Split(size_t startIndex,
                                         size_t count) const noexcept {
    assert(startIndex + count <= m_size);
    return SparseArrayView<T>(
        array_view_details::AdvancePointer<true>(m_p, startIndex), count);
  }
...
[[nodiscard]] inline constexpr DenseArrayView<T> MakeArrayView(
    T (&arr)[size]) noexcept {
  return DenseArrayView<T>(arr, size);
}

template <typename T, typename Member, size_t size>
[[nodiscard]] inline constexpr SparseArrayView<Member> MakeArrayView(
    T (&arr)[size], Member T::*member) noexcept {
