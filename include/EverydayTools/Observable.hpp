...
#include "EverydayTools\Exception\ThrowIfFailed.hpp"

template <typename T>
using RawPtr = T*;

template <typename Derived, typename Listener,
          template <typename> typename StorePtr = RawPtr>
class Observable {
 public:
  void Subscribe(StorePtr<Listener> listener) {
    CallAndRethrow("Observable::Subscribe", [&] {
      auto it =
          std::lower_bound(m_listeners.begin(), m_listeners.end(), listener);
      edt::ThrowIfFailed(it == m_listeners.end() || *it != listener,
