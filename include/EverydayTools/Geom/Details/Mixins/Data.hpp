...
class Mixin {
 public:
  T& At(size_t nRow, size_t nColumn) noexcept { return data[nRow][nColumn]; }

  const T& At(size_t nRow, size_t nColumn) const noexcept {
    return data[nRow][nColumn];
  }

  T* GetData() { return &data[0][0]; }

  const T* GetData() const { return &data[0][0]; }
  ...
  LineMatrixView<std::add_const_t<T>, nRows, 1> GetColumn(size_t column) const {
    ...
    if constexpr (nRows == 1) {
      ...
      return LineMatrixView<std::add_const_t<T>, nRows, 1>(&data[0][column],
                                                           nColumns);
    }
  }
