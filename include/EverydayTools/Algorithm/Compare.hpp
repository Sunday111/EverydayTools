...
constexpr WorstReference_t<Arg0, Arg1, Args...> CompareValues(
    Comparator&& comparator, Arg0&& a0, Arg1&& a1, Args&&... args) noexcept {
  if constexpr (sizeof...(Args) == 0) {
    return comparator(a0, a1) ? std::forward<Arg0>(a0) : std::forward<Arg1>(a1);
  } else {
    return CompareValues(
        std::forward<Comparator>(comparator), std::forward<Arg0>(a0),
        CompareValues(std::forward<Comparator>(comparator),
                      std::forward<Arg1>(a1), std::forward<Args>(args)...));
  }
}
}  // namespace edt::detail
...
constexpr decltype(auto) MinInteger(Arg0&& a0, Arg1&& a1,
                                    Args&&... args) noexcept {
  using namespace detail;
  static_assert(pureSame<Arg0, Arg1, Args...>);
  const MinIntegerComparator comparator;
  return CompareValues(comparator, std::forward<Arg0>(a0),
                       std::forward<Arg1>(a1), std::forward<Args>(args)...);
}

template <typename Arg0, typename Arg1, typename... Args>
constexpr decltype(auto) MaxInteger(Arg0&& a0, Arg1&& a1,
                                    Args&&... args) noexcept {
  using namespace detail;
  static_assert(pureSame<Arg0, Arg1, Args...>);
